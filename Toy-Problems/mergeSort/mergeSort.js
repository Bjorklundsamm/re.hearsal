...
class MergeSort {
  ...
  splitArr(array) {
    ...
    this.mergeWhole(this.currentArray);
  }

  mergeWhole(array) {
    console.log(array);
    if (array.length === 1) {
      // this.currentArray = array[0];
      return;
    }
    var result = [];
    for (var i = 0; i < array.length; i+=2){
      var mergedUnit = this.mergeTwo(array[i], array[i + 1]);
      result.push(mergedUnit);
    }
    this.mergeWhole(result);
  }

  mergeTwo(arr1, arr2) {
    var tempArr = arr1.slice();
    for (var value of arr2) {
      for (var n = tempArr.length; n > 0; n--) {
        if (value < tempArr[n]) {
          if (tempArr.length === 1) {
